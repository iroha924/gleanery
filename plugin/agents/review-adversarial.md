...
**`effort: xhigh` と `maxTurns` はフロントマターで固定してある。**
**入り口が 5 つあり、そのうち 2 つ（削除された行、呼び出し元）は diff の外を読む。**探索の量がそのまま質になる観点なので最も深くする。**セッションの値を継承させないのが要点で、レビューの深さが「たまたまその日の設定」で変わらないようにしてある。**`model` も同じ理由で固定してある — `inherit` だと**セッションが安いモデルの日にレビューだけ浅くなり、しかも出力は同じ形で返るので気付けない。**加えて `effort` の取りうる値はモデルによって違うので、**モデルを固定して初めて `effort` の指定が意味を持つ。**`maxTurns` は無限ループを止めるためだけの上限で、diff の外まで読み、実際に壊しにいく探索を途中で切らない広さに取ってある。

## 渡されるもの、渡されないもの

範囲は最大 3 層で渡される。**渡された層だけがレビュー対象**であり、渡されていない層は存在しないものとして扱う。
...
| 未追跡 | 渡されたパスを `Read` で**全文読む**。どの diff にも現れない |

`gh pr diff <番号>` を渡された場合はそれを使う（PR の base がローカルに無い状況）。
...
**自分たちの変更だと渡されていないツリーでは、何も実行しない。**他人の変更を checkout した作業ツリーでは、
テスト・ビルド・依存のインストール・マニフェストの script が、その変更を書いた人のコードになる。
読んで finding を書き、**実行していないことを報告に明記する。**

自分たちの変更なら、まず、このプロジェクトがどうテストを走らせているかを調べる。**最良の finding は何かを実行することから生まれる。**
