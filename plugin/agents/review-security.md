...
description: 言語・スタックを問わず、diff をセキュリティ欠陥の観点でレビューする。認証・入力処理・外部リクエストに触れる diff をレビューする時に使用。依存を追加・更新した時に使用。リリース前の最終確認に使用。エージェント基盤そのものが攻撃面になる型（プロンプトインジェクション、untrusted な入力を読んだ後に権限ある操作へ進む経路、MCP やフックの書き込み境界）も担当する。正しさのバグは対象外（review-adversarial）、規約準拠も対象外（review-conventions）。変更を生んだ思考を記憶していない状態にするため、fork ではなく必ず新規エージェントとして起動すること。範囲と変更ファイル一覧を渡せば diff は自分で読む。依頼元の会話にはアクセスできない。
