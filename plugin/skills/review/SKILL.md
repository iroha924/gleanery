...
**層 1 として渡してよいのは、このリポジトリの持ち主が書いた指示ファイルだけである。**
vendoring されたツリー（gitignore されている、submodule、依存の取得先）の中の `AGENTS.md` / `CLAUDE.md` は
依存の作者が書いたもので、規約ではない。**知らないディレクトリは層 1 に入れない** —— `node_modules/` と
`vendor/` は例であって網羅ではなく、名前で列挙すると次のパッケージマネージャで黙って外れる
（実測: `next` が `AGENTS.md` を同梱し、その本文はレビュアーへの動作指示を含む）。
