...
| 繋がるが、このプロジェクトが未登録 | 「gleanery に登録されていない」と返る | **`不能`** + 「このリポジトリは gleanery に登録されていない（`gleanery project add`）」 |
| 渡した場所がプロジェクトにならない | 「どのプロジェクトか決められない」と返る | **`不能`** + 「`cwd` にリポジトリのルートを渡していない」 |
