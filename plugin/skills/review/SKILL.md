...
塞ぐなら `sandbox.enabled`（Bash とその子プロセスの到達範囲を OS が強制する）。
**`permissionMode` は使えない** — プラグイン由来の agent はこのフィールドを無視する。
...
| **Codex** | `claude -p "<定義の本文>" --output-format json` |
...
上限は 3 ラウンドで、3 ラウンド目の裁定の後も直すべき指摘が残っていれば、残った指摘と各裁定を利用者に示して
判断を求める。何を直すべき指摘とするかは `code-review-triage` の規律に従う。

2 ラウンド目以降の範囲は、**前のラウンドで見た HEAD から今の HEAD までの差分**（Step 1 の 3 層を、base を
前のラウンドの HEAD にして取る）と、**変更全体のファイル一覧**にする。直しは差分の外の呼び出し元を壊しうるので、
一覧は差分に絞らない。

**前のラウンドで直した finding の一覧も渡す。**1 件ごとに要約・場所・直した commit を書く。レビュアーには、
それが本当に解けたかと、直しとその呼び出し元に新しい欠陥が無いかを見させる。**一覧は確かめる対象であって、
見る範囲を狭めるものではない**と書き、新しい欠陥も探させる。**`REFUTED` にした finding と棄却の理由は渡さない。**
