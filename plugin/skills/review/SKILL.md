...
`ReportFindings` が使えるなら、それで返す。ホストの UI がその形で描画する。
**`verdict` は `CONFIRMED` / `PLAUSIBLE` の 2 値**なので、`REFUTED` はそこに載らない。

**`outcome` に逃がさない。**あれは「**修正を当てた後の再報告**」専用の欄で、
このスキルは直さないのでその条件を満たさない。**`REFUTED` は台帳の隣に別の節として残す** —
崩れた指摘と、崩した根拠を 1 行ずつ。**棄却したことを残さないと、次のラウンドで同じ指摘が来て、
再評価の費用を払い直すことになる。**
