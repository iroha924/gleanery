...
| 繋がるが、この作業場所が未登録 | 「gleanery に登録されていない」と返る | **`不能`** + 「このリポジトリは gleanery に登録されていない（`gleanery project add`）」 |
...
```
✦ **gleanery review** · origin/main...HEAD · 観点 5 × モデル 2

| 観点（立てた型） | Claude | Codex |
|---|---|---|
| 正しさ（`gleanery:review-adversarial`） | ✓ 実行（2 件） | ✗ 不能（利用上限） |
| セキュリティ（`gleanery:review-security`） | ✓ 実行（0 件） | ✗ 不能（利用上限） |
| 明文化された規約（`gleanery:review-conventions`） | ✓ 実行（1 件） | ✗ 不能（利用上限） |
| 冗長さ（`gleanery:review-cleanup`） | △ 打ち切り（1 件まで） | ✗ 不能（利用上限） |
| 過去の判断（`gleanery:review-precedent`） | ✓ 実行（0 件） | ✗ 不能（利用上限） |
