...
背景で立てたレーンの報告は 1 件ずつ届き、届いた時点で文脈に入る。**読めても裁定は始めない** —
台帳の全レーンが `実行` / `打ち切り` / `不能` のどれかになってから Step 5 へ進む。
...
**再現の伴わない指摘だけを `review-validator` へ回す。**validator もレビュアーと同じく背景で立てる
（前面では報告が持ち主の画面に出ない）。報告者が既に再現しているものは、
...
画面に出なかった）。**フックが効かない経路では、起動側が生の報告を 1 レーンずつ文章で出す** — 前面で立てた agent、Claude から回す
Codex のレーン（`codex exec`）、Codex がホストのとき（Codex の plugin にはフックが無い）。そのうえで、畳んで裁定した
結果を返す。1 万字を超える報告は、画面には先頭と、全文を逃がしたファイルの場所だけが出る（Claude Code の上限）。
