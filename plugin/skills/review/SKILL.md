...
**渡すのは範囲と変更ファイル一覧だけ。**diff はレビュアーが自分で読む。
...
**別モデルのレーンを回すのは、新しい変更への 1 ラウンド目だけ。**指摘を直した commit を確かめる
2 ラウンド目以降は、自分の側のレーンだけで回す。台帳には相手のレーンを `未実行` と「直しの確認のラウンド」で残す。
別モデルの目が効くのは新しい diff の盲点で、直しの確認に同じ費用を払い直さない（実測 2026-09-13:
Codex の 5 レーンは 1 ラウンドに 104〜172 万トークンを使い、指摘は 57 → 29 → 3 件と減った。直しが持ち込んだ
後退は、その後の Claude だけのラウンドでも捕まった）。直しが新しい仕組みを足すほど大きいなら、それは新しい変更として
1 ラウンド目から回す。相手のレーンを回さないラウンドでも、Step 6 の `CONFIRMED` は再現できたときだけにする
（下の「入っていなければ」と同じく、モデルの多様性が無い分を再現で埋める）。
...
印と状態名を並べて書くのは、この凡例と下の例の表だけにする（`scripts/check-pairs.mjs` が CLI の印と突き合わせる）。
