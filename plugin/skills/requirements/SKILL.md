...
allowed-tools: Read, Edit(.mitos/**), AskUserQuestion, Bash(${CLAUDE_PLUGIN_ROOT}/bin/mitos check*), mcp__plugin_mitos_mitos__current_work, mcp__plugin_mitos_mitos__search_knowledge, mcp__plugin_mitos_mitos__check_path
---
...
| 操作 | 書く順序 |
|---|---|
| change を新しく作る | `change.json` を `{"schema": "mitos/change/1", "title": "<題>", "requirements": {"status": "draft"}}` で書く → `requirements.md` を作る |
...
## Step 2 — 一問ずつ聞いて draft を直す

最も影響の大きい未解決事項を 1 つ選び、推奨案・理由・主要な代替案と不利な点を添えて聞く。
利用者が答える前に、独立した別の判断をまとめて聞かない。
...
回答を反映して `requirements.md` を直す。節と根拠の書き分けは [references/template.md](references/template.md) に従う。
新しい要求が出たら元の要求の節へ足し、Loop を続ける。
...
> 元の要求とのずれ、観測できない受け入れ条件、根拠の無い断定、解決方法の混入。引用できない指摘は書くな。
> ファイルは編集するな。
