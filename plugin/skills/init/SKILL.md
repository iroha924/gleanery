---
name: init
description: このリポジトリに、要件定義と設計書の置き場所（.mitos/）を mitos init で作る。作った場所が想定したリポジトリの根かを利用者に確かめさせる。要件の整理は始めない。
disable-model-invocation: true
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/bin/mitos init*)
---
...
| サブディレクトリに作る | 同期も `mitos check` もリポジトリの根しか見ないので、成果物がどこからも引けない |
...
素の `mitos` は使わない。Codex では PATH に無く、Claude Code では PATH の CLI が古い版のことがあり
...
2. 出力の 1 行目にある作成先を、そのまま利用者へ示す。「作った」と「既に初期化済み」を区別して伝える
3. 作成先が想定したリポジトリの根かを利用者に確かめさせる。違っていても自分で消したり作り直したりしない
4. 出力に同期経路の更新の注意があれば、そのまま伝える
5. ここで止まる。要件定義は利用者が `/mitos:requirements`（Codex は `$mitos:requirements`）で始める
