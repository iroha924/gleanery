...
create table gleanery.person (
  id bigint generated always as identity primary key,
  display_name text not null unique check (display_name <> ''),
  is_self boolean not null default false
);
-- 質問者本人は 1 人だけ。「私はなんて言った？」の「私」をここで決める。
create unique index person_one_self on gleanery.person (is_self) where is_self;

-- 取り込み元での識別子。GitHub の user id は login を変えても同じなので external_id に置く。
create table gleanery.person_identity (
  id bigint generated always as identity primary key,
  person_id bigint references gleanery.person (id) on delete set null,
  provider text not null check (provider in ('github')),
  external_id text not null,
  handle text not null,
  unique (provider, external_id)
);
create index person_identity_handle on gleanery.person_identity (provider, lower(handle));
...
create table gleanery.connector (
  id bigint generated always as identity primary key,
  project_id bigint not null references gleanery.project (id) on delete cascade,
...
create table gleanery.source_item (
  id bigint generated always as identity primary key,
  ...
  body text,
  author_identity_id bigint references gleanery.person_identity (id) on delete set null,
...
create table gleanery.conversation (
  id uuid primary key,
  project_id bigint not null references gleanery.project (id) on delete cascade,
  source_item_id bigint references gleanery.source_item (id) on delete cascade,
...
create index conversation_recent on gleanery.conversation (project_id, started_at desc);
...
create table gleanery.message (
  id uuid primary key,
  conversation_id uuid not null references gleanery.conversation (id) on delete cascade,
  external_id text not null,
  turn_id text,
  reply_to_id uuid references gleanery.message (id) on delete set null,
  speaker_kind text not null check (speaker_kind in ('self', 'person', 'assistant', 'bot')),
  identity_id bigint references gleanery.person_identity (id) on delete set null,
...
create index message_order on gleanery.message (conversation_id, sent_at);
create index message_by_identity on gleanery.message (identity_id, sent_at desc) where identity_id is not null;
create index message_self on gleanery.message (sent_at desc) where speaker_kind = 'self';
create index message_lexemes on gleanery.message using gin (lexemes);
...
create table gleanery.message_file (
  message_id uuid not null references gleanery.message (id) on delete cascade,
...
create index message_file_path on gleanery.message_file (path);

-- 埋め込みは本体と分ける。本体を書き換えても索引の heap を巻き込まず、失敗も状態として残す。
-- 書き戻しは source_hash が一致するときだけ（古い結果が後から返っても上書きしない）。
create table gleanery.message_embedding (
  message_id uuid primary key references gleanery.message (id) on delete cascade,
  model text not null,
...
create table gleanery.work_item (
  id bigint generated always as identity primary key,
  project_id bigint not null references gleanery.project (id) on delete cascade,
  ...
  conversation_id uuid references gleanery.conversation (id) on delete set null,
  updated_at timestamptz not null,
  unique (project_id, source_key)
);
create index work_item_open on gleanery.work_item (project_id, updated_at desc)
...
create table gleanery.knowledge (
  id bigint generated always as identity primary key,
  project_id bigint not null references gleanery.project (id) on delete cascade,
  source_item_id bigint references gleanery.source_item (id) on delete cascade,
  conversation_id uuid references gleanery.conversation (id) on delete cascade,
  work_item_id bigint references gleanery.work_item (id) on delete set null,
  source_key text not null,
  ...
  decision_id bigint references gleanery.knowledge (id) on delete cascade,
  superseded_by_id bigint references gleanery.knowledge (id) on delete set null,
  heading text,
  body text not null check (body <> ''),
  reason text,
  confirmation text,
...
create index knowledge_listing on gleanery.knowledge (project_id, kind, status, occurred_at desc);
create index knowledge_lexemes on gleanery.knowledge using gin (lexemes);
create index knowledge_work on gleanery.knowledge (work_item_id) where work_item_id is not null;

-- 判断とファイルの直接の関係。applies_to は編集の前に出す制約、evidence は根拠として挙げたファイル。
create table gleanery.knowledge_file (
  knowledge_id bigint not null references gleanery.knowledge (id) on delete cascade,
  path text not null check (path <> '' and path !~ '^/' and path !~ '(^|/)\.\.(/|$)'),
...
create index knowledge_file_path on gleanery.knowledge_file (path, role);

create table gleanery.knowledge_embedding (
  knowledge_id bigint primary key references gleanery.knowledge (id) on delete cascade,
  model text not null,
  source_hash bytea not null check (octet_length(source_hash) = 32),
  status text not null check (status in ('pending', 'ready', 'error')),
  attempts integer not null default 0,
...
declare
  r text;
begin
  foreach r in array array['gleanery_reader', 'gleanery_ingest', 'gleanery_capture'] loop
...
grant usage on schema extensions, gleanery to gleanery_reader, gleanery_ingest, gleanery_capture;

grant select on all tables in schema gleanery to gleanery_reader;

grant select, insert, update, delete on all tables in schema gleanery to gleanery_ingest;
grant usage on all sequences in schema gleanery to gleanery_ingest;
...
grant select (id, key, name) on gleanery.project to gleanery_capture;
grant select (id) on gleanery.message to gleanery_capture;
grant insert (id, project_id, origin, external_id, branch, started_at) on gleanery.conversation to gleanery_capture;
grant insert (id, conversation_id, external_id, turn_id, speaker_kind, body, truncated, original_bytes, sent_at,
              content_hash, lexemes) on gleanery.message to gleanery_capture;
grant insert (message_id, path, action) on gleanery.message_file to gleanery_capture;
grant insert (message_id, model, source_hash, status, embedding) on gleanery.message_embedding to gleanery_capture;
