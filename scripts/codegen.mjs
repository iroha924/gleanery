...
// db/compose.yaml と同じ image を使う。ここがずれると、手元と違う PostgreSQL の型が入る。
const IMAGE = "pgvector/pgvector:0.8.6-pg18";
const NAME = "gleanery-codegen";
...
try {
  ...
  const out = check ? `${OUT}.check` : OUT;
  execFileSync(
    path.join(root, "server/node_modules/.bin/kysely-codegen"),
    ["--dialect", "postgres", "--out-file", out, "--include-pattern", "gleanery.*"],
    {
      env: {
        ...env,
  ...
  if (check) {
    ...
    fs.rmSync(out);
    const want = fs.existsSync(OUT) ? fs.readFileSync(OUT, "utf8") : null;
    if (want !== got) {
      console.error(
        want === null
          ? `${path.relative(root, OUT)} が無い。\`bun run codegen\` で作る`
          : `${path.relative(root, OUT)} が db/schema.sql と合っていない。\`bun run codegen\` で作り直す`,
      );
      process.exit(1);
    }
    console.log(`${path.relative(root, OUT)} は db/schema.sql と一致している`);
  } else {
    console.log(`${path.relative(root, OUT)} を db/schema.sql から作り直した`);
  }
} finally {
  remove();
}
