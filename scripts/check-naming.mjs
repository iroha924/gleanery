...
import { formerNameLines, hasFormerName } from "./lib/former-name.mjs";
...
for (const file of tracked) {
  ...
  if (hasFormerName(file)) hits.push(`${file}  former product name in the file name`);
  ...
  for (const n of formerNameLines(body)) hits.push(`${file}:${n}  former product name`);
}
...
if (hits.length) {
  ...
  console.error(
    "\nIf there is a reason to keep a pattern hit, add it with the reason to ALLOWED in scripts/check-naming.mjs. The former product name has no exceptions.",
  );
  process.exit(1);
}
