...
- **作業中に引く。** MCP のツール `recall` と `read` で、Claude Code と Codex が過去の決定・棄却した案・制約・行き止まりと、あなたが以前のセッションで言ったことを検索します。
...
- **頼まれたときに判断を残す。** `/sphica:trace` で、そのセッションの決定・棄却した案・制約・行き止まりを、会話に出てきた PR・issue とあわせて保存し、作業の現在地も残します。
...
- **PR から判断を残す。** `/sphica:harvest <番号>` で GitHub の PR を 1 つ、レビューのコメントや後から足したコミットまで読み、決まったことを同じ形で保存します。何を判断とみなすかはエージェントが決めるので、PR のテンプレートを問いません。
...
`~/.sphica/sphica.db` を作り、このリポジトリを登録します。もう一度打っても、DB にも登録にも触りません。`origin` の remote が無いリポジトリは、名前を付けて登録します: `sphica init --name <名前>`。
...
PR で決まったことを残すには、Claude Code で `/sphica:harvest 123`（Codex では `$sphica:harvest 123`）を打ちます。番号を付けなければ、最近の PR を出してどれにするかを聞きます。
...
- **通信。** アカウントも外部のサービスもテレメトリもなく、Sphica 自身は通信しません。ほかのツールを呼ぶものが 2 つあり、そのツールが通信することはあります。`/sphica:harvest` は PR を読むためにあなたの資格情報で `gh api` を、`sphica doctor` は入っているバージョンを確かめるために `npm` と `claude` を実行します。
- **ほかの人が書いた文章。** `/sphica:harvest` で読む PR の文章は誰でも書けます。エージェントにはデータとして渡し、MCP サーバーは DB に書き込めません。harvest を保存するコマンドが書くのは、いまのリポジトリのその PR の記録だけです。
...
| `sphica doctor` | バージョン・DB・記録と、プロジェクトごとの最後の harvest を確かめる |
| `sphica advice` | 編集のフックが制約を出した割合を見る |

ふだん使うこれらは `sphica --help`、プロジェクトの管理、エージェントや保守で使うものも含めた全部の一覧は `sphica -H`、各コマンドのオプションは `sphica <コマンド> --help` で見られます。
