...
| `plugin/agents/`の各レビュアーがuntrustedとして名指しする列挙 | 最小集合を全員が含む（超過は可） | `scripts/check-pairs.mjs` |
| `Bash`を持つレビュアーの「他人のツリーでは何も実行しない」の段落 | `Bash`を持つ全定義にあること | `scripts/check-pairs.mjs` |
