...
- 既定値を URL に出さない。`?page=1` のような既定は消す
- 画面をまたいで保つ検索語やページ番号を、**他の画面へ漏らさない**
...
空と失敗を同じ表示で済ませない — 利用者の次の行動が変わる。分岐に使う API 名は `ui-hono` Skill。

- **何ができなかったかを先に書く。**例外の文字列だけを出さない（`String(error)` は
  `Error: /api/... が 500` だけを見せる）。この道具は持ち主 1 人が使い、利用者と開発者が同じ人なので、
  **詳細は隠さず補助として添えてよい** — 「セッションの一覧を読めませんでした（/api/sessions が 500）」の形
- 本文を出す領域へ失敗を流し込まない。読み手が本文かエラーかを区別できない
- 黙って捨てる（`.catch(() => {})`）のは、利用者が気付かなくてよい失敗だけにし、理由をコードに書く
- **`toast` は失敗だけに使う。**成功は結果そのものが画面に出る（録音が始まれば表示が変わり、
  文字起こしは文字が流れ込む）ので、重ねて知らせる意味が無い。実測で `toast.success` は 0 件
- `toast` を使うのは、**結果が画面に出ない失敗**に限る（マイクが使えない、画面共有が始まらない）。
  画面に出せる失敗は、その場所へ出す
...
**ビルドが通ることは最適化された証拠にならない。**設定・診断の取り方・移行の作法は `ui-hono` Skill。
