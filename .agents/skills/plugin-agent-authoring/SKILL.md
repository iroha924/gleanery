...
**`Bash`は再現に要る観点（正しさ・データ損失、セキュリティ）と裁定役にだけ渡す。**渡すと書き込みが止まらない
（実測: `Read`と`Bash`だけのレビュアーがファイルを作った）ので、他人のツリーではこの 3 つを起動しない（`plugin/skills/review/SKILL.md`の
ツールの表と「塞ぐ手段は無い」が正本）。ほかの観点は`Read` / `Grep` / `Glob`だけにし、`git`を実行できない分、差分はファイルで渡す。
