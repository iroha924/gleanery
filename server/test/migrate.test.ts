...
test("DB の版より新しい migration だけを、revision の昇順で返す", () => {
  ...
  assert.deepEqual(admin.pendingMigrations(files, 3), [
    { revision: 4, file: "0004_b.sql" },
    { revision: 5, file: "0005_c.sql" },
  ]);
  assert.deepEqual(admin.pendingMigrations(files, 2), [
...
test("DB が最後の migration の版か、それより新しければ何も返さない", () => {
  ...
  assert.deepEqual(admin.pendingMigrations(files, 4), []);
  assert.deepEqual(admin.pendingMigrations(files, 5), []);
});
...
test("名前が NNNN_<英小文字・数字・_>.sql でないファイルがあれば、DB が最新でもその名前を挙げて投げる", () => {
  for (const bad of ["004_b.sql", "0004-b.sql", "0004_B.sql", "0004.sql", "0004_b.sql~", "README.md"]) {
    assert.throws(
      () => admin.pendingMigrations(["0003_a.sql", bad], 3),
      (e: unknown) => {
...
test("同じ revision の migration が 2 本あれば、DB がその版を過ぎていても両方の名前を挙げて投げる", () => {
  for (const current of [2, 4]) {
    assert.throws(
      () => admin.pendingMigrations(["0003_a.sql", "0004_b.sql", "0004_c.sql"], current),
      (e: unknown) => {
        assert.ok(
...
test("DB の次の版から最後までに欠番があれば、欠けた revision を挙げて投げる", () => {
  assert.throws(() => admin.pendingMigrations(["0003_a.sql", "0005_c.sql"], 2), /(?<!\d)0*4(?!\d)/);
  assert.throws(() => admin.pendingMigrations(["0004_b.sql"], 2), /(?<!\d)0*3(?!\d)/);
});
