...
| **agent の報告を見る** | 表示のフック（Claude Code） | 背景の agent が終わったとき、その報告の本文を画面に出す |
...
- agent の報告は記録しないので、登録していない作業場所でも出る。行頭の `│ ` は agent が書いた本文の印。
  1 万字を超えると先頭だけが出る。前面の agent と Codex では出ない
