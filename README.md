...
設計と現在地は `MEMORY.md` の「最重要: JARVIS」にある。
...
| **人** | 名簿。`@shogo-kurokawa-nm` → 黒川さん のような呼び名と、**質問者本人が誰か**を明示的に設定する |
| **言葉** | 社内語の用語集。チャットが分からない語に出会うと `ask_term` で登録候補に積む |

**人と言葉は推測させず、人間が明示的に設定する。**これがあると「黒川さんはなんて言ってた？」
...
`server/evals/` に 4 種類ある。**LLM を審判にしていない** — 人間との一致は 90% と報告されているが
審判自体の校正が要り、実行のたびに揺れる。答えには PR 番号・日付・状態という検証可能な語が
必ず入るので、突き合わせで足りる。

| 束 | 中身 |
|---|---|
| `hybrid.json` | 検索の recall（30 問） |
| `answers.json` | 手書きの事実（GitHub / Linear / DB で裏を取ったもの） |
| `answers-auto.json` | DB から機械生成（複合条件、近い番号の干渉、過去と現在） |
...
- `mustNot: ["OT-4858"]` と書いたが OT-4858 は OT-4856 の子で、正しく挙げただけで不合格になった
...
設計の経緯・踏んだ失敗の型・いまの弱点は `MEMORY.md` にある。
