...
found by message search (recall mode: said) and `sphica search --said`.
...
Lines in the form `- Chosen: <option>. Rejected: <option> (<reason>); <option> (<reason>)` are imported as Sphica
